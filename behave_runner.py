...
def run_behave_allure_command():
    ...
    environment_features = f"features{os.sep}feature_domains{os.sep}{env_selected}"
    ...
    result = behave_main(
        f"{tags}-f allure_behave.formatter:AllureFormatter -o {allure_results_env} -k {environment_features}"
    )
    if result == 1:
        raise BaseException
